...
public class PicDao {
	...
	public void insert(Pic pic) throws SQLException{
		...
		pstmt.setBinaryStream(5, pic.getPicData(),pic.getPicSize());
	 ...
	 * 返回图片集合
	 * */
	public List<Pic> query() throws Exception{
		...
		while(rs.next()){
			...
			Pic pic = new Pic(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getLong(4),rs.getString(5),null,rs.getTimestamp(7));
	 ...
	 * 根据id返回图片二进制数据
	 * */
	public Pic queryById(int picId) throws Exception{
		Connection conn = DbUtil.getConnection();
